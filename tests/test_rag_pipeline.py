...
def test_stream_rag_answer() -> None:
    ...
    with (
        patch("utils.rag_pipeline.get_embeddings", return_value=[[0.1, 0.2]]),
        patch(
            "utils.rag_pipeline.stream_chat",
            return_value=iter(["Generated", " answer", " text."]),
        ),
    ):
        ...
        sources = next(item["content"] for item in items if item["type"] == "sources")
